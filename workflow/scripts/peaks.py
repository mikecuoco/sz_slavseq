...
class Peak:
	...
	def add_read(self, r):
		...
		if r.reference_end > self.end:
			self.coverage = np.append(self.coverage, np.zeros(r.reference_end - self.end))
			self.end = r.reference_end  
			self.width = self.end - self.start
...
for r in bam.fetch():
	...
	if (
		...
		and (r.get_tag("YA") > 20 and r.get_tag("YG") < 10)
