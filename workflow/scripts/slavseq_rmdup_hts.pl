...
prio_pair_rmdup $input_bam_fn_abspath, 'selected.txt';

system("samtools view -H $input_bam_fn_abspath > header.txt"); # get header only
# sort input bam by first column, output lines identical to selected.txt, and append header to create output bam file 
system(qq[samtools view $input_bam_fn_abspath | sort -T ./ -S 1500M -s -k 1,1 | join -t "\t" - selected.txt | cat header.txt - | samtools view -S -b - > $output_bam_fn]);
# chdir $curdir;
