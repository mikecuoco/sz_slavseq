...
class TrainTest:
    def __init__(self, sample=None, exp="", input_type="pickle"):
        self.sample = (
            snakemake.wildcards.donor
        )  # maybe change self.sample to self.donor
    ...
    def train_classifier(self, features, labels):
        cla = RandomForestClassifier(
            bootstrap=True, n_estimators=100, oob_score=True, n_jobs=-1
        )
    ...
    def run_train_test(self, features, pred, phase, model=None, le=None, labels=None):
        ...
        if phase == "Training":
            model = self.train_classifier(features, labels)
        ...
        pred = self.get_ypred(
            pred, pd.DataFrame(proba), features["all_reads.count"], le
        )

        log_stdout = open(self.outDir + "/%s_report_performance.txt" % phase, "w+")
        print("\n Report performance %s" % phase)
        self.report_performance(
            model, features, labels, predictions, log_stdout, phase, le
        )

        log_stdout = open(
            self.outDir + "/%s_report_feature_importance.txt" % phase, "w+"
        )
    ...
    def run(self):
        ...
        for k in range(snakemake.params.num_folds):
            ...
            self.inFiles = [
                Path(f) for f in snakemake.input if re.search(f"fold_{k}", f)
            ]
            ...
            if self.get_groups(train_y).count() != self.get_groups(test_y).count():
                log_stdout = open(
                    self.outDir + "/" + self.sample + "_has_no_equal_classes.txt", "w+"
                )
                print(
                    "\nThis cell (%s) fold (%i) has no enough classes to classifier... "
                    % (self.sample, k),
                    file=log_stdout,
                    flush=True,
                )
                print(
                    ...
                    % (
                        self.get_groups(train_y).count(),
                        self.get_groups(test_y).count(),
                    ),
                    file=log_stdout,
                    flush=True,
                )
            ...
            print("\nTraining the model... ")
            Train, df, trained_model = self.run_train_test(
                train_x.copy(), train_y.copy(), phase="Training", le=le, labels=labels
            )
            ...
            Test, df, trained_model = self.run_train_test(
                test_x.copy(),
                test_y.copy(),
                phase="Testing",
                model=trained_model,
            ...
            df.to_csv(
                self.outDir + "/Train_Test_Accuracy.csv",
                sep=";",
                index=False,
                header=True,
            )
    ...
    def report_feature_importances(self, cla, feature_names, log_stdout):
        ...
        for i in range(len(ii)):
            print(
                "",
                "\t".join([str(cla.feature_importances_[ii[i]]), feature_names[ii[i]]]),
                file=log_stdout,
                flush=True,
            )
    ...
    def report_performance(self, cla, features, labels, pred, log_stdout, title, le):
        ...
        self.pretty_plot_confusion_matrix(
            df_cm,
            accuracy=acc,
            fz=fz,
            cmap=cmap,
    ...
    def plot_confusion_matrix(
        self, cm, normalize=False, title="Confusion matrix", cmap=plt.cm.Blues, fn="cm"
    ):
        """
