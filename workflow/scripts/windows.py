    ...
    def get_reads(self, contig):
        ...
        for r in self.bam.fetch(contig, multiple_iterators=True):
            if self.read_filter:
                ...
                yield Read(
                    r.reference_name, r.reference_start, r.reference_end, r.is_reverse
                )

    def run(self, contigs: list = None):
        """
...
class SlidingWindows(BaseWindows):
    def __init__(
        self,
    ...
    def test(self, peak_windows, bg_windows: dict, pval_cutoff=10e-5):
        """
        ...
        while True:
            ...
            p["fold_change"] = (
                np.float16(p[f"count_{k}"] / mu)
                if mu > 0
                else np.float16(p[f"count_{k}"])
            )
    ...
    def call_peaks(self, contig: str):
        ...
        # get peak windows
        peak_windows = self.window(
            contig, self.window_size, self.step_size, min_reads=self.min_reads
        )
        ...
        if self.bg_sizes:
            ...
            peaks = self.test(peak_windows, bg_windows1)

            # merge overlapping peaks
            if self.merge:
                merged = self.merge(peaks)

                bg_windows2 = {ws: self.window(contig, ws) for ws in self.bg_sizes}

                return self.test(merged, bg_windows2)
            else:
                return peaks
        else:
            if self.merge:
                return self.merge(peak_windows)
            else:
                return peak_windows


class OverlapWindows(BaseWindows):
