...
if [ $1 == "chm13v2" ]; then
	wget -O- --tries=10 $url13v2 | gzip -dc > $1.fa 2> /dev/null
elif [ $1 == "hs38DH" ]; then
	(wget -O- --tries=10 $url38 | gzip -dc; cat $tmp/bwa.kit/resource-GRCh38/hs38DH-extra.fa) > $1.fa 
	# [ ! -f $1.fa.alt ] && cp $tmp/bwa.kit/resource-GRCh38/hs38DH.fa.alt $1.fa.alt
elif [ $1 == "hs38a" ]; then
	wget -O- --tries=10 $url38 | gzip -dc > $1.fa
	# [ ! -f $1.fa.alt ] && grep _alt $tmp/bwa.kit/resource-GRCh38/hs38DH.fa.alt > $1.fa.alt
elif [ $1 == "hs38" ]; then
	wget -O- --tries=10 $url38 | gzip -dc | awk '/^>/{f=/_alt/?0:1}f' > $1.fa
elif [ $1 == "hs37d5" ]; then
	wget -O- --tries=10 $url37d5 | gzip -dc > $1.fa 2>/dev/null
elif [ $1 == "hs37" ]; then
	wget -O- --tries=10 $url37d5 | gzip -dc 2>/dev/null | awk '/^>/{f=/>hs37d5/?0:1}f' > $1.fa
else
