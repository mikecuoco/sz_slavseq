...
def my_cluster_id_cell(df):
    ...
    for x in df[["chrom", "start", "end", "sample", "cell_id"]].itertuples():
        ...
        if (
            prev_iv is None
            or not overlaps(iv, prev_iv)
            or prev_cell is None
            or prev_cell != (x[4], x[5])
        ):
            cluster_id += 1
        yield cluster_id
        prev_iv = iv
...
def write_insertions(outDir, df0, sample, window_size):
    ...
    if len(df0) > 0:
        ...
        df = slavseq_sz.sort_values(
            ["sample", "cell_id", "chrom", "start", "end"]
        ).copy()
        ...
        cs = (
            ...
            .size()
            .to_frame()
            .rename(columns={0: "number_cells_detected"})
        )
        ...
        df = df[
            [
                ...
                "proba_max",
                "reads_max",
                "number_cells_detected",
                "cluster_id",
            ]
        ]

        df = df.rename(
            columns={"proba_max": "confidence_score", "reads_max": "number_reads"}
        )
        df.to_csv(
            outDir + "/slavseq_sz-intersections-cluster.csv", index=True, header=True
        )

        df = df[(df["number_cells_detected"] <= 5)].sort_values(
            ["sample", "cell_id", "confidence_score"], ascending=[False, False, False]
        )
...
def get_results_metrics(inDir, sample):
    ...
    for k in range(0, snakemake.params.num_folds):
        ...
        for line in lines:
            ...
            if line == lines[0]:
                X2[tFile + "_precision"] = float(a.replace("precision ", ""))
            if line == lines[1]:
                X2[tFile + "_recall"] = float(a.replace("recall ", ""))
            if line == lines[2]:
                X2[tFile + "_f1"] = float(a.replace("f1 ", ""))
...
def main():
    ...
    df_metrics.to_csv(
        outDir + "/Cross_validation_metrics.csv", index=False, header=True
    )
