...
def distance(read1, read2):
    if (read1["sam"][1] & 4) != 0 or (read2["sam"][1] & 4) != 0:
        return np.inf
    return distance0(
        read1["sam"][2], start_pos(read1), read2["sam"][2], start_pos(read2)
    )
...
def secondary_read_count(alignments, min_secondary_mapq):
    ...
    return len(
        set(
            x["sam"][0]
            for x in alignments
            if (x["sam"][1] & 2304) != 0 and x["sam"][4] >= min_secondary_mapq
        )
    )
...
class WindowFeatures:
    ...
    def r2_starts_and_strands_(self, alignments):
        ...
        return [
            ...
            if ((x["sam"][1] & 2304) == 0)
            and ((x["sam"][1] & 128) != 0)
            and x["sam"][2] == self.chrom
            and self.start <= x["sam"][3] <= self.end
        ]
    ...
    def features0(self, alignments):
        ...
        feature_dict["median_ya"] = (
            np.nan
            if feature_dict["count"] == 0
            else np.median([x["tags"]["YA"] for x in primary_r1_reads])
        )
        ...
        feature_dict["r1r2_distance_1k_200k.count"] = len(
            [x for x in d if 1000 <= x <= 200000]
        )

        sd = secondary_read_distances(alignments, self.min_secondary_mapq)
        feature_dict["secondary.median_distance"] = (
            np.nan if len(sd) == 0 else np.median(sd)
        )

        feature_dict["secondary_distance_1k_200k.count"] = len(
            [x for x in d if 1000 <= x <= 200000]
        )
        ...
        (peakpos, peakcount, peak_te_strand) = self.read_start_peak_features(
            positions_and_strands
        )
        ...
        if self.ensearch is not None:
            (motif, en_pos, en_score) = self.ensearch.pos_and_score(
                self.chrom, peakpos, peak_te_strand
            )
    ...
    def features(self):
        ...
        alignments_from_well_mapped_yayg_reads = list(
            primary_read_filter(
                alignments_from_well_mapped_reads,
                lambda x: x["tags"]["YA"] >= self.min_ya
                and x["tags"]["YG"] <= self.max_yg,
            )
        )
        ...
        alignments_from_well_mapped_yg_reads = list(
            primary_read_filter(
                alignments_from_well_mapped_reads,
                lambda x: x["tags"]["YG"] <= self.max_yg,
            )
        )
